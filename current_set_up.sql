...
create table INVENTORY(Bloodbag_id int, Bank_id int, primary key (Bloodbag_id, Bank_id), 
foreign key (Bloodbag_id) references BLOODBAG(id_Bloodbag), 
foreign key (Bank_id) references BANK(id_Bank));

create table VACCOUNT(id_Account int, Vampire_id int not null, preference int, 
